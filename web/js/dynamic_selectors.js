...
function install(n, normalize, restoreInputs) {
...
app.registerExtension({ name: "ArtemKo7v.UsefulStuffNodes.DynamicSelectors", nodeCreated(n) {
    if (n.constructor.type === ANY) install(n, normalizeAny, (x,d) => restore(x,d,["any_"], (node,i) => add(node, `any_${i}`, "*")));
    if (n.constructor.type === PAIRS) install(n, normalizePairs, (x,d) => restore(x,d,["image_", "text_"], addPair));
}});