...
- **Any Input Selector** — selects one connected input (up to 64). `mode` defaults to `fixed`, which uses the 1-based `selected_index` (default `1`). `random` chooses a connected input on each execution, with repeats allowed. `increment` / `decrement` start at `selected_index`, then move forward / backward on each execution, wrapping at either end. Indexes count connected inputs in socket order, skipping disconnected optional sockets. In cycling modes, an invalid starting index becomes `1`; changing the mode, starting index, or connected input slots restarts the cycle. Progress is kept on the running node instance, including repeated API requests; it is not saved in the workflow and resets when the instance is recreated (for example, after a server restart). For compatibility, `fixed` retains the old random fallback for a missing or out-of-range index, including `-1` in older workflows.
